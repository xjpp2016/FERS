...
class FeatureRestorationNet(nn.Module):
    def __init__(self, channels, num_blocks=3):
        super(FeatureRestorationNet, self).__init__()
        layers = []
...
if __name__ == "__main__":
    model = HyperSpatialResNet(input_channels=100)
    dummy_input = torch.randn(4, 100, 224, 224)
    out, _ = model(dummy_input)
    print("final output shape:", out.shape)
    model = FeatureRestorationNet(channels=100)
    out = model(out)
    print("final output shape:", out.shape)